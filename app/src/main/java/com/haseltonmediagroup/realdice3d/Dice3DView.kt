...
class Dice3DView @JvmOverloads constructor(
    ...
    private class DiceRenderer : Renderer {
        ...
        private val dice = arrayOf(
            DieState(px = -1.05f, pz = -0.8f),
            DieState(px = 1.05f, pz = -1.2f, rx = 90f)
        )
        ...
        // Fully closed rounded-box mesh. Six subdivided faces share the same rounded surface.
        ...
        override fun onSurfaceChanged(
            ...
            Matrix.perspectiveM(projection, 0, 42f, ratio, 1f, 50f)
            Matrix.setLookAtM(
                view, 0,
                0f, 7.4f, 11.8f,
                0f, -0.15f, -0.8f,
        ...
        fun roll(force: Float) {
            ...
            // Start near the camera and finish deep into the tray so perspective changes are obvious.
            val leftEnd = Random.nextFloat() * 1.15f - 1.95f
            val rightEnd = Random.nextFloat() * 1.15f + 0.80f
            val endZ = floatArrayOf(
                Random.nextFloat() * 1.45f - 3.15f,
                Random.nextFloat() * 1.65f - 1.85f
            )

            dice.forEachIndexed { index, d ->
                d.value = Random.nextInt(1, 7)
                d.startNs = now
                d.duration = (1.75f + force * 0.30f + index * 0.11f).coerceAtMost(2.65f)
                d.startPx = if (index == 0) -1.55f else 1.55f
                d.startPz = 3.65f + index * 0.28f
                d.targetPx = if (index == 0) leftEnd else rightEnd
                ...
                d.spinX = Random.nextInt(1050, 1750) * force * if (index == 0) 1f else -1f
                d.spinY = Random.nextInt(900, 1600) * force * if (index == 0) -1f else 1f
                d.spinZ = Random.nextInt(650, 1250) * force
                d.curve = (Random.nextFloat() * 1.0f + 0.5f) * if (index == 0) 1f else -1f
                d.phase = index * 0.42f
                d.lastBounce = -1
                d.rolling = true
            }
        ...
        private fun updateAnimation() {
            ...
            dice.forEachIndexed { index, d ->
                if (!d.rolling) return@forEachIndexed
                anyRolling = true
                rollingCount++
                ...
                // Strong forward depth travel, with a curved path and a small rebound near the end.
                val travelEase = 1f - (1f - t) * (1f - t)
                val settleEase = 1f - (1f - t) * (1f - t) * (1f - t)
                d.px = lerp(d.startPx, d.targetPx, travelEase) +
                    sin(t * PI.toFloat()) * d.curve * (1f - 0.55f * t)
                d.pz = lerp(d.startPz, d.targetPz, travelEase) +
                    sin(t * PI.toFloat() * 1.35f + d.phase) * 0.42f * (1f - t)

                // Four diminishing physical-looking bounces. The die returns to table level at each impact.
                val bounceCycles = 4.25f
                val bouncePhase = t * bounceCycles
                val bounce = abs(sin(bouncePhase * PI.toFloat()))
                val envelope = (1f - t) * (1f - 0.30f * t)
                d.lift = (2.45f + 0.28f * rollForce) * envelope * bounce +
                    0.12f * sin(t * PI.toFloat())

                // Trigger audio/haptic events on successive table contacts.
                val bounceIndex = floor(bouncePhase).toInt()
                if (bounceIndex > d.lastBounce && bounceIndex > 0 && t < 0.96f) {
                    d.lastBounce = bounceIndex
                    val strength = (0.90f - bounceIndex * 0.16f + rollForce * 0.06f).coerceIn(0.16f, 1f)
                ...
                if (t >= 1f) {
                    ...
                    d.rz = d.targetRz
                    d.rolling = false
                    onImpact?.invoke(0.15f)
                }
            }

            if (rollingCount > 0) averageT /= rollingCount
            if (!collisionTriggered && anyRolling && averageT > 0.34f) {
                collisionTriggered = true
                // A subtle mid-flight die-to-die click; paths remain separated enough to avoid overlap.
                onImpact?.invoke(0.52f)
            }
        ...
        private fun drawTray() {
            ...
            drawGeometry(
                ...
                trayNormals,
                6,
                floatArrayOf(0.035f, 0.125f, 0.085f, 1f),
                0.38f
            )
        ...
        private fun drawRim(x: Float, y: Float, z: Float, sx: Float, sz: Float, sy: Float) {
            ...
            drawGeometry(
                ...
                cubeNormals,
                cubeVertexCount,
                floatArrayOf(0.13f, 0.085f, 0.05f, 1f),
                0.22f
            )
        }

        private fun drawShadow(d: DieState) {
            val radius = 0.64f + d.lift * 0.035f
            val verts = circleOnPlane(0f, -0.645f, 0f, radius, 34)
            ...
            drawGeometry(
                ...
                buffer(normals),
                verts.size / 3,
                floatArrayOf(0.002f, 0.003f, 0.003f, alpha),
                0f,
        ...
        private fun drawDie(d: DieState) {
            ...
            // V4: approximately 20% smaller than V3 to leave more visible travel space.
            Matrix.scaleM(model, 0, 0.58f, 0.58f, 0.58f)
            ...
            drawGeometry(
                ...
                cubeNormals,
                cubeVertexCount,
                floatArrayOf(0.965f, 0.95f, 0.90f, 1f),
                0.92f
            )
            drawAllPips()
        }
        ...
        private fun drawPip(face: Face, u: Float, v: Float) {
            ...
            fun point(index: Int, a: Float, b: Float) {
                ...
                normals[index * 3] = nn[0]
                normals[index * 3 + 1] = nn[1]
                normals[index * 3 + 2] = nn[2]
            }
            ...
            for (i in 0..segments) {
                ...
                point(
                    i + 1,
                    u + radius * cos(a).toFloat(),
                    v + radius * sin(a).toFloat()
                )
            }
            drawGeometry(
                buffer(verts),
                buffer(normals),
                segments + 2,
                floatArrayOf(0.018f, 0.019f, 0.020f, 1f),
                0.15f,
                GLES20.GL_TRIANGLE_FAN
            )
        }
        ...
        companion object {
            ...
            private const val FRAGMENT_SHADER = """
                ...
                void main() {
                    ...
                    float lighting = 0.52 + diffuse * uLightStrength;
                    vec3 rgb = uColor.rgb * lighting + vec3(spec * 0.17 * uLightStrength);
            ...
            private fun buildRoundedCube(subdivisions: Int, radius: Float): Pair<FloatArray, FloatArray> {
                ...
                fun roundedPoint(x: Float, y: Float, z: Float): Pair<FloatArray, FloatArray> {
                    ...
                    val ny = dy / len
                    val nz = dz / len
                    return floatArrayOf(qx + nx * radius, qy + ny * radius, qz + nz * radius) to
                        floatArrayOf(nx, ny, nz)
                }

                fun addVertex(p: FloatArray, n: FloatArray) {
                ...
                fun addTri(
                    ...
                    c: Pair<FloatArray, FloatArray>,
                    expected: FloatArray
                ) {
                    val cr = cross(a.first, b.first, c.first)
                    if (dot(cr, expected) >= 0f) {
                        addVertex(a.first, a.second); addVertex(b.first, b.second); addVertex(c.first, c.second)
                    } else {
                        addVertex(a.first, a.second); addVertex(c.first, c.second); addVertex(b.first, b.second)
                    }
                }

                data class FaceSpec(val axis: Int, val sign: Float, val expected: FloatArray)
                val faces = listOf(
                    FaceSpec(0, 1f, floatArrayOf(1f, 0f, 0f)),
                    FaceSpec(0, -1f, floatArrayOf(-1f, 0f, 0f)),
                    FaceSpec(1, 1f, floatArrayOf(0f, 1f, 0f)),
                    FaceSpec(1, -1f, floatArrayOf(0f, -1f, 0f)),
                    FaceSpec(2, 1f, floatArrayOf(0f, 0f, 1f)),
                    FaceSpec(2, -1f, floatArrayOf(0f, 0f, -1f))
                )

                fun rawPoint(spec: FaceSpec, u: Float, v: Float): Pair<FloatArray, FloatArray> {
                    val p = when (spec.axis) {
                        0 -> floatArrayOf(spec.sign, u, v)
                        1 -> floatArrayOf(u, spec.sign, v)
                        else -> floatArrayOf(u, v, spec.sign)
                    }
                ...
                for (spec in faces) {
                    for (iy in 0 until subdivisions) {
                        val v0 = -1f + 2f * iy / subdivisions
                        val v1 = -1f + 2f * (iy + 1) / subdivisions
                        for (ix in 0 until subdivisions) {
                            val u0 = -1f + 2f * ix / subdivisions
                            val u1 = -1f + 2f * (ix + 1) / subdivisions
                            val p00 = rawPoint(spec, u0, v0)
                            val p10 = rawPoint(spec, u1, v0)
                            val p11 = rawPoint(spec, u1, v1)
                            val p01 = rawPoint(spec, u0, v1)
                            addTri(p00, p10, p11, spec.expected)
                            addTri(p00, p11, p01, spec.expected)
                        }
                    }
                }
            ...
            private fun circleOnPlane(
                ...
                for (i in 0..segments) {
                    val a = 2.0 * PI * i / segments
                    val j = (i + 1) * 3
                    out[j] = cx + cos(a).toFloat() * radius
                    out[j + 1] = y
                    out[j + 2] = cz + sin(a).toFloat() * radius
                }
                return out
            }
